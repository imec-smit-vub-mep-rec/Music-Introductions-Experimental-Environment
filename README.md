...
## Public overview of the introductions

`/review` is a public page listing every song per genre, with its informative or immersive introduction (toggle) shown in the original participant player (`AudioPlayerScreen`): the lyrics scroll along with the spoken introduction. The full transcript is shown below it. The selected song and style are kept in the URL (e.g. `/review?song=blues_1&style=immersive`), so links to a specific introduction can be shared.

The song recordings are not published: the songs in `public/data` are anonymized placeholders, so when an introduction ends the page shows the song's title and artist instead of playing it (via `onIntroductionComplete` on `AudioPlayerScreen`). Session tracking is disabled on `/review` (see `getSession` in `src/lib/session.ts`), so browsing it never changes a participant session stored in the same browser.
